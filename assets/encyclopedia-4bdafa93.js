import{H as l}from"./star-sky-2bdbce32.js";const q=`{{#each this}}
...
{{/each}}`,D=document.querySelector(".sw-era"),x="http://localhost:3000/",N="starWars";async function P(){const n=await(await fetch(`${x}${N}`)).json(),t=l.compile(q);D.innerHTML=t(n.encyclopedia.eras)}P();const A=`{{#each this}}
...
{{/each}}`,H=document.querySelector(".sw-factions"),J="http://localhost:3000/",O="starWars";async function W(){const n=await(await fetch(`${J}${O}`)).json(),t=l.compile(A);H.insertAdjacentHTML("beforeend",t(n.encyclopedia.factions))}W();const F=`{{#each this}}
<li data-form="{{key}}">
    {{name}}
</li>
{{/each}}`,V=document.querySelector(".forms-list"),G="http://localhost:3000/",Y="starWars";async function K(){const n=await(await fetch(`${G}${Y}`)).json(),t=l.compile(F);V.innerHTML=t(n.encyclopedia.lightsaberForms)}K();const Q=`<h2 class="sw-planets__title sw-section-title">
...
</ul>`,R="http://localhost:3000/",_="starWars",z=document.querySelector(".sw-planets");let y={},i=[],S=null,k=null;async function U(){try{y=(await(await fetch(`${R}${_}`)).json()).encyclopedia,i=y.planets,X()}catch(e){console.log(e)}}function X(){const e=l.compile(Q);z.innerHTML=e(i),Z()}function Z(){document.querySelector(".create-planet-form").addEventListener("submit",ee),document.querySelector(".sw-planets__list").addEventListener("click",te)}async function ee(e){e.preventDefault();const t=document.querySelector(".planet-name").value.trim();t&&(i.push({id:Date.now(),name:t}),await L(),e.target.reset())}function te(e){const n=e.target.closest(".edit-btn"),t=e.target.closest(".delete-btn");if(n){k=Number(n.dataset.id);const a=i.find(({id:d})=>Number(d)===k),s=prompt("Нова назва",a.name);if(!s)return;a.name=s,L()}t&&(S=Number(t.dataset.id),i=i.filter(({id:a})=>Number(a)!==S),L())}async function L(){try{y.planets=i,await fetch(`${R}${_}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({encyclopedia:y})}),U()}catch(e){console.log(e)}}U();const ne=`{{#each this}}
<article
...
{{/each}}`,ae=document.querySelector(".sw-characters__groups"),se="http://localhost:3000/",oe="starWars";async function ie(){const n=await(await fetch(`${se}${oe}`)).json(),t=l.compile(ne);ae.innerHTML=t(n.encyclopedia.characterGroups)}ie();const ce=`{{#each this}}
