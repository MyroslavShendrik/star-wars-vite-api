import{H as f}from"./timer-4c11cdbd.js";const x=`{{#each this}}
...
{{/each}}`,B=document.querySelector(".sw-eras__container"),N="http://localhost:3000/",H="starWars";async function A(){try{const t=await(await fetch(`${N}${H}`)).json(),n=f.compile(x);B.innerHTML=n(t.encyclopedia.eras)}catch(e){console.log(e)}}A();const F=`{{#each this}}
...
{{/each}}`,O=document.querySelector(".sw-factions"),J="http://localhost:3000/",W="starWars";async function V(){const t=await(await fetch(`${J}${W}`)).json(),n=f.compile(F);O.insertAdjacentHTML("beforeend",n(t.encyclopedia.factions))}V();const G=`{{#each this}}
<li data-form="{{key}}">
    {{name}}
</li>
{{/each}}`,Y=document.querySelector(".forms-list"),K="http://localhost:3000/",z="starWars";async function Q(){const t=await(await fetch(`${K}${z}`)).json(),n=f.compile(G);Y.innerHTML=n(t.encyclopedia.lightsaberForms)}Q();const X=`<h2 class="sw-planets__title sw-section-title">
...
<ul class="sw-planets__list">
...
</ul>`,M="http://localhost:3000/",_="starWars",Z=document.querySelector(".sw-planets");let h={},c=[],y=null,q=null;async function U(){console.log("1. getPlanets()");try{const e=await fetch(`${M}${_}`);if(!e.ok)throw new Error(`HTTP error: ${e.status}`);const t=await e.json();console.log("Отримали JSON:",t),h=t.encyclopedia,c=h.planets||[],console.log("Планети:",c),k()}catch(e){console.error("Помилка getPlanets:",e)}}function k(e=c){console.log("2. renderPlanets()"),console.log("Малюємо:",e);const t=f.compile(X);Z.innerHTML=t(e),ee(),me()}function ee(){console.log("3. addListeners()");const e=document.querySelector(".create-planet-form"),t=document.querySelector(".sw-planets__list"),n=document.querySelector("#searchPlanet"),o=document.querySelector("#searchBtn");e||console.error("❌ .create-planet-form НЕ знайдений"),t||console.error("❌ .sw-planets__list НЕ знайдений"),e&&e.addEventListener("submit",te),t&&t.addEventListener("click",ne),n&&n.addEventListener("input",ie),o&&o.addEventListener("click",de)}function te(e){e.preventDefault(),console.log("4. Натиснуто Додати");const t=document.querySelector(".planet-name");if(!t){console.error("❌ .planet-name не знайдений");return}const n=t.value.trim();if(!n){alert("Введіть назву планети."),t.focus();return}console.log("Назва нової планети:",n),q=null,$("create",{name:n,climate:"",terrain:"",gravity:"",population:"",description:""})}function ne(e){const t=e.target.closest(".edit-btn"),n=e.target.closest(".delete-btn");if(t){e.stopPropagation();const o=Number(t.dataset.id);console.log("5. EDIT. ID:",o);const a=c.find(r=>Number(r.id)===o);if(!a){console.error("❌ Планету не знайдено:",o);return}q=o,$("edit",a);return}if(n){e.stopPropagation();const o=Number(n.dataset.id);console.log("6. DELETE. ID:",o);const a=c.find(r=>Number(r.id)===o);if(!a){console.error("❌ Планету не знайдено:",o);return}y=o,ce(a);return}}function oe(){let e=document.querySelector("#planetCrudModal");if(e)return e;console.log("Створюємо CRUD modal"),e=document.createElement("div"),e.id="planetCrudModal",e.className="modal",e.innerHTML=`
  ...
  `,document.body.appendChild(e),e.querySelector(".planet-delete-close").addEventListener("click",L),e.querySelector(".planet-delete-cancel").addEventListener("click",L),e.querySelector(".planet-delete-confirm").addEventListener("click",le),e.addEventListener("click",t=>{t.target===e&&L()}),e)}function ce(e){console.log("9. Відкриваємо delete modal");const t=re(),n=t.querySelector(".planet-delete-text");n.textContent=`Ви дійсно хочете видалити планету "${e.name}"?`,t.style.display="block"}function L(){const e=document.querySelector("#planetDeleteModal");e&&(e.style.display="none",y=null)}async function le(){if(console.log("10. Підтвердження DELETE:",y),y===null){console.error("❌ ID для видалення відсутній");return}const e=c.findIndex(t=>Number(t.id)===Number(y));if(e===-1){console.error("❌ Планету для видалення не знайдено");return}c.splice(e,1),await T(),L(),alert("Планету успішно видалено!")}async function T(){console.log("11. savePlanets()");try{h.planets=c,console.log("PATCH:",`${M}${_}`),console.log("Відправляємо:",h);const e=await fetch(`${M}${_}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({encyclopedia:h})});if(!e.ok)throw new Error(`PATCH error: ${e.status}`);const t=await e.json();console.log("PATCH результат:",t),await U()}catch(e){console.error("❌ Помилка savePlanets:",e),alert("Не вдалося зберегти зміни. Перевір json-server.")}}let R;function ie(e){clearTimeout(R);const t=e.target.value.trim().toLowerCase();R=setTimeout(()=>{console.log("12. Пошук:",t),D(t)},1500)}function de(){const e=document.querySelector("#searchPlanet");if(!e)return;const t=e.value.trim().toLowerCase();console.log("12. Пошук кнопкою:",t),D(t)}function D(e){if(!e){k(c);return}const t=c.filter(n=>n.name.toLowerCase().includes(e));console.log("Результат пошуку:",t),k(t)}function me(){console.log("13. addPlanetModalListeners()");const e=document.querySelectorAll(".planet-card"),t=document.getElementById("planetModal"),n=document.getElementById("modalTitle"),o=document.getElementById("modalText"),a=document.querySelector(".modal__close");if(!t){console.error("❌ #planetModal не знайдений");return}e.forEach(r=>{r.addEventListener("click",async l=>{if(l.target.closest(".edit-btn")||l.target.closest(".delete-btn"))return;const u=Number(r.dataset.id);console.log("14. Відкриваємо планету:",u);const s=c.find(i=>Number(i.id)===u);if(!s){console.error("Планету не знайдено");return}n.textContent=s.name,o.innerHTML=`
        ...
        `,t.style.display="block"})}),a&&(a.onclick=()=>{t.style.display="none"}),t.onclick=r=>{r.target===t&&(t.style.display="none")}}console.log("🚀 planets.js запущений");U();const ue=`{{#each this}}
<article
...
{{/each}}`,pe=document.querySelector(".sw-characters__groups"),fe="http://localhost:3000/",ge="starWars";async function he(){const t=await(await fetch(`${fe}${ge}`)).json(),n=f.compile(ue);pe.innerHTML=n(t.encyclopedia.characterGroups)}he();const ye=`{{#each this}}
...
function $e(){if(!E.length)return;const e=E.map(t=>Ue(t));Promise.allSettled(e).then(()=>{console.log("Усі зірки активні")})}//! запуск
Re();$e();
