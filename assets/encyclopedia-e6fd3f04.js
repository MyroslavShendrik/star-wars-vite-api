import{H as w,g as K,c as j}from"./timer-674c392f.js";const z=`{{#each this}}
...
{{/each}}`,ne=document.querySelector(".sw-factions"),ae="http://localhost:3000/",oe="starWars";async function re(){const t=await(await fetch(`${ae}${oe}`)).json(),n=w.compile(te);ne.insertAdjacentHTML("beforeend",n(t.encyclopedia.factions))}re();const se=`{{#each this}}
<li data-form="{{key}}">
    {{name}}
</li>
{{/each}}`,le=document.querySelector(".forms-list"),ce="http://localhost:3000/",ie="starWars";async function de(){const t=await(await fetch(`${ce}${ie}`)).json(),n=w.compile(se);le.innerHTML=n(t.encyclopedia.lightsaberForms)}de();var me="Expected a function",O=0/0,ue="[object Symbol]",pe=/^\s+|\s+$/g,fe=/^[-+]0x[0-9a-f]+$/i,ge=/^0b[01]+$/i,he=/^0o[0-7]+$/i,ye=parseInt,be=typeof j=="object"&&j&&j.Object===Object&&j,ve=typeof self=="object"&&self&&self.Object===Object&&self,Le=be||ve||Function("return this")(),we=Object.prototype,Se=we.toString,Ee=Math.max,Ie=Math.min,$=function(){return Le.Date.now()};function Te(e,t,n){var a,o,s,i,l,r,m=0,p=!1,f=!1,u=!0;if(typeof e!="function")throw new TypeError(me);t=F(t)||0,R(n)&&(p=!!n.leading,f="maxWait"in n,s=f?Ee(F(n.maxWait)||0,t):s,u="trailing"in n?!!n.trailing:u);function h(d){var y=a,S=o;return a=o=void 0,m=d,i=e.apply(S,y),i}function v(d){return m=d,l=setTimeout(L,t),p?h(d):i}function T(d){var y=d-r,S=d-m,H=t-y;return f?Ie(H,s-S):H}function b(d){var y=d-r,S=d-m;return r===void 0||y>=t||y<0||f&&S>=s}function L(){var d=$();if(b(d))return A(d);l=setTimeout(L,T(d))}function A(d){return l=void 0,u&&a?h(d):(a=o=void 0,i)}function V(){l!==void 0&&clearTimeout(l),m=0,a=r=o=l=void 0}function Y(){return l===void 0?i:A($())}function P(){var d=$(),y=b(d);if(a=arguments,o=this,r=d,y){if(l===void 0)return v(r);if(f)return l=setTimeout(L,t),h(r)}return l===void 0&&(l=setTimeout(L,t)),i}return P.cancel=V,P.flush=Y,P}function R(e){var t=typeof e;return!!e&&(t=="object"||t=="function")}function je(e){return!!e&&typeof e=="object"}function qe(e){return typeof e=="symbol"||je(e)&&Se.call(e)==ue}function F(e){if(typeof e=="number")return e;if(qe(e))return O;if(R(e)){var t=typeof e.valueOf=="function"?e.valueOf():e;e=R(t)?t+"":t}if(typeof e!="string")return e===0?e:+e;e=e.replace(pe,"");var n=ge.test(e);return n||he.test(e)?ye(e.slice(2),n?2:8):fe.test(e)?O:+e}var Me=Te;const ke=K(Me),_e=`<h2 class="sw-planets__title sw-section-title">
...
<ul class="sw-planets__list">
...
</ul>`,U="http://localhost:3000/",D="starWars",Ce=document.querySelector(".sw-planets");let g={},c=[],E=null,C=null;async function W(){console.log("1. getPlanets()");try{const e=await fetch(`${U}${D}`);if(!e.ok)throw new Error(`HTTP error: ${e.status}`);const t=await e.json();console.log("Отримали JSON:",t),g=t.encyclopedia,c=g.planets||[],c.sort((n,a)=>n.name.localeCompare(a.name)),console.log("Планети:",c),N()}catch(e){console.error("Помилка getPlanets:",e)}}function N(e=c){console.log("2. renderPlanets()"),console.log("Малюємо:",e);const t=w.compile(_e);Ce.innerHTML=t(e),Pe(),$e(),Je()}function Pe(){const e=document.querySelector(".planet-select");if(!e){console.error("❌ .planet-select не знайдений");return}e.innerHTML=`
    <option value="">Оберіть планету</option>
  `,g.availablePlanets.forEach(t=>{e.insertAdjacentHTML("beforeend",`
        <option value="${t.id}">
          ${t.name}
        </option>
      `)})}function $e(){console.log("3. addListeners()");const e=document.querySelector(".create-planet-form"),t=document.querySelector(".sw-planets__list"),n=document.querySelector("#searchPlanet"),a=document.querySelector("#searchBtn"),o=document.querySelector(".add-available-planet-btn");e||console.error("❌ .create-planet-form НЕ знайдений"),t||console.error("❌ .sw-planets__list НЕ знайдений"),e&&e.addEventListener("submit",xe),o&&o.addEventListener("click",Re),t&&t.addEventListener("click",Ue),n&&n.addEventListener("input",Fe),a&&a.addEventListener("click",We)}function xe(e){e.preventDefault(),console.log("4. Натиснуто Додати");const t=document.querySelector(".planet-name");if(!t){console.error("❌ .planet-name не знайдений");return}const n=t.value.trim();if(!n){alert("Введіть назву планети."),t.focus();return}console.log("Назва нової планети:",n),C=null,J("create",{name:n,climate:"",terrain:"",gravity:"",population:"",description:""})}async function Re(){console.log("Додаємо планету з availablePlanets");const e=document.querySelector(".planet-select");if(!e){console.error("❌ .planet-select не знайдений");return}const t=e.value;if(!t){alert("Оберіть планету зі списку.");return}const n=g.availablePlanets.find(a=>Number(a.id)===Number(t));if(!n){console.error("❌ Планету не знайдено:",t);return}console.log("Обрана планета:",n),c.push(n),g.availablePlanets=g.availablePlanets.filter(a=>Number(a.id)!==Number(t)),c.sort((a,o)=>a.name.localeCompare(o.name)),console.log("Планети:",c),console.log("Доступні планети:",g.availablePlanets),await M(),alert(`Планету "${n.name}" успішно додано!`)}function Ue(e){const t=e.target.closest(".edit-btn"),n=e.target.closest(".delete-btn");if(t){e.stopPropagation();const a=Number(t.dataset.id);console.log("5. EDIT. ID:",a);const o=c.find(s=>Number(s.id)===a);if(!o){console.error("❌ Планету не знайдено:",a);return}C=a,J("edit",o);return}if(n){e.stopPropagation();const a=Number(n.dataset.id);console.log("6. DELETE. ID:",a);const o=c.find(s=>Number(s.id)===a);if(!o){console.error("❌ Планету не знайдено:",a);return}E=a,He(o);return}}function De(){let e=document.querySelector("#planetCrudModal");if(e)return e;console.log("Створюємо CRUD modal"),e=document.createElement("div"),e.id="planetCrudModal",e.className="modal",e.innerHTML=`
  ...
  `,document.body.appendChild(e);const t=e.querySelector(".planet-crud-close"),n=e.querySelector(".planet-cancel-btn"),a=e.querySelector(".planet-crud-form");return t.addEventListener("click",I),n.addEventListener("click",I),e.addEventListener("click",o=>{o.target===e&&I()}),a.addEventListener("submit",Ne),e}function J(e,t){console.log("7. openPlanetFormModal()",e,t);const n=De(),a=n.querySelector(".planet-crud-title"),o=n.querySelector(".crud-name"),s=n.querySelector(".crud-climate"),i=n.querySelector(".crud-terrain"),l=n.querySelector(".crud-gravity"),r=n.querySelector(".crud-population"),m=n.querySelector(".crud-description");e==="create"?a.textContent="Додати нову планету":a.textContent="Редагувати планету",o.value=t.name||"",s.value=t.climate||"",i.value=t.terrain||"",l.value=t.gravity||"",r.value=t.population||"",m.value=t.description||"",n.dataset.mode=e,n.style.display="block",o.focus()}function I(){console.log("Закриваємо CRUD modal");const e=document.querySelector("#planetCrudModal");if(!e)return;e.style.display="none";const t=e.querySelector(".planet-crud-form");t&&t.reset(),C=null}async function Ne(e){e.preventDefault(),console.log("8. Submit CRUD form");const t=e.target,a=document.querySelector("#planetCrudModal").dataset.mode,o=t.querySelector(".crud-name").value.trim(),s=t.querySelector(".crud-climate").value.trim(),i=t.querySelector(".crud-terrain").value.trim(),l=t.querySelector(".crud-gravity").value.trim(),r=t.querySelector(".crud-population").value.trim(),m=t.querySelector(".crud-description").value.trim();if(!o){alert("Введіть назву планети.");return}if(o.length<2){alert("Назва планети повинна містити мінімум 2 символи.");return}if(!s){alert("Введіть клімат планети.");return}if(!i){alert("Введіть тип поверхні планети.");return}if(!l){alert("Введіть гравітацію планети.");return}if(r===""){alert("Введіть населення планети.");return}const p=Number(r);if(!Number.isInteger(p)||p<0){alert("Населення повинно бути цілим числом не менше 0.");return}if(!m){alert("Введіть опис планети.");return}const f={name:o,climate:s,terrain:i,gravity:l,population:String(p),description:m};if(console.log("Дані планети:",f),a==="create"){const u={id:Be(),...f};console.log("Створюємо:",u),c.push(u),await M(),I();const h=document.querySelector(".create-planet-form");h&&h.reset(),alert("Планету успішно додано!");return}if(a==="edit"){const u=c.findIndex(h=>Number(h.id)===Number(C));if(u===-1){console.error("❌ Не знайшли планету для редагування");return}c[u]={...c[u],...f},console.log("Оновлена планета:",c[u]),await M(),I(),alert("Планету успішно відредаговано!")}}function Be(){const e=c.map(n=>Number(n.id));return(e.length?Math.max(...e):0)+1}function Ae(){let e=document.querySelector("#planetDeleteModal");return e||(console.log("Створюємо Delete modal"),e=document.createElement("div"),e.id="planetDeleteModal",e.className="modal",e.innerHTML=`
  ...
  `,document.body.appendChild(e),e.querySelector(".planet-delete-close").addEventListener("click",q),e.querySelector(".planet-delete-cancel").addEventListener("click",q),e.querySelector(".planet-delete-confirm").addEventListener("click",Oe),e.addEventListener("click",t=>{t.target===e&&q()}),e)}function He(e){console.log("9. Відкриваємо delete modal");const t=Ae(),n=t.querySelector(".planet-delete-text");n.textContent=`Ви дійсно хочете видалити планету "${e.name}"?`,t.style.display="block"}function q(){const e=document.querySelector("#planetDeleteModal");e&&(e.style.display="none",E=null)}async function Oe(){if(console.log("10. Підтвердження DELETE:",E),E===null){console.error("❌ ID для видалення відсутній");return}const e=c.findIndex(n=>Number(n.id)===Number(E));if(e===-1){console.error("❌ Планету для видалення не знайдено");return}const t=c[e];console.log("Видаляємо планету:",t),c.splice(e,1),g.availablePlanets.push(t),console.log("Доступні планети після видалення:",g.availablePlanets),await M(),q(),alert("Планету успішно видалено!")}async function M(){console.log("11. savePlanets()");try{g.planets=c,console.log("PATCH:",`${U}${D}`),console.log("Відправляємо:",g);const e=await fetch(`${U}${D}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({encyclopedia:g})});if(!e.ok)throw new Error(`PATCH error: ${e.status}`);const t=await e.json();console.log("PATCH результат:",t),await W()}catch(e){console.error("❌ Помилка savePlanets:",e),alert("Не вдалося зберегти зміни. Перевір json-server.")}}const Fe=ke(e=>{const t=e.target.value.trim().toLowerCase();console.log("12. Пошук:",t),G(t)},3e3);function We(){const e=document.querySelector("#searchPlanet");if(!e)return;const t=e.value.trim().toLowerCase();console.log("12. Пошук кнопкою:",t),G(t)}function G(e){if(!e){N(c);return}const t=c.filter(n=>n.name.toLowerCase().includes(e));console.log("Результат пошуку:",t),N(t)}function Je(){console.log("13. addPlanetModalListeners()");const e=document.querySelectorAll(".planet-card"),t=document.getElementById("planetModal"),n=document.getElementById("modalTitle"),a=document.getElementById("modalText"),o=document.querySelector(".modal__close");if(!t){console.error("❌ #planetModal не знайдений");return}e.forEach(s=>{s.addEventListener("click",async i=>{if(i.target.closest(".edit-btn")||i.target.closest(".delete-btn"))return;const l=Number(s.dataset.id);console.log("14. Відкриваємо планету:",l);const r=c.find(m=>Number(m.id)===l);if(!r){console.error("Планету не знайдено");return}n.textContent=r.name,a.innerHTML=`
          <p>
            <b>Climate:</b>
            ${r.climate}
          </p>
        ...
        `,t.style.display="block"})}),o&&(o.onclick=()=>{t.style.display="none"}),t.onclick=s=>{s.target===t&&(t.style.display="none")}}console.log("🚀 planets.js запущений");W();const Ge=`{{#each this}}
<article
...
function pt(e){return new Promise(t=>{const n=500+Math.random()*3e3,a=["#fff","#ffd700","#87ceeb","#ff69b4"],o=a[Math.floor(Math.random()*a.length)];e.style.background=o;//! старт мінімальна яскравість
