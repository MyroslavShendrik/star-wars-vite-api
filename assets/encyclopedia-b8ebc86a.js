import{H as l}from"./timer-c89a4807.js";const x=`{{#each this}}
...
</ul>`,U="http://localhost:3000/",B="starWars",Z=document.querySelector(".sw-planets");let y={},i=[],R=null,T=null;async function $(){try{y=(await(await fetch(`${U}${B}`)).json()).encyclopedia,i=y.planets,ee()}catch(e){console.log(e)}}function ee(){const e=l.compile(X);Z.innerHTML=e(i);const t=document.getElementById("searchBtn");console.log("searchBtn:",t),te()}function te(){document.querySelector(".create-planet-form").addEventListener("submit",ne),document.querySelector(".sw-planets__list").addEventListener("click",ae)}async function ne(e){e.preventDefault();const n=document.querySelector(".planet-name").value.trim();n&&(i.push({id:Date.now(),name:n}),await L(),e.target.reset())}function ae(e){const t=e.target.closest(".edit-btn"),n=e.target.closest(".delete-btn");if(t){T=Number(t.dataset.id);const a=i.find(({id:d})=>Number(d)===T),s=prompt("Нова назва",a.name);if(!s)return;a.name=s,L()}n&&(R=Number(n.dataset.id),i=i.filter(({id:a})=>Number(a)!==R),L())}async function L(){try{y.planets=i,await fetch(`${U}${B}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({encyclopedia:y})}),$()}catch(e){console.log(e)}}$();const se=`{{#each this}}
<article
...
function je(){const e=v.map(t=>ke(t));Promise.allSettled(e).then(()=>{console.log("Усі зірки активні")})}Se();je();const r=document.getElementById("planetModal"),b=document.getElementById("modalTitle"),I=document.getElementById("modalText"),Re=document.querySelector(".modal__close"),Te=document.querySelectorAll(".planet-card"),M=document.getElementById("searchPlanet"),S=document.getElementById("searchBtn");console.log("searchBtn:",S);function C(e){b.textContent=e.name,I.innerHTML=`
    <p><b>Climate:</b> ${e.climate}</p>
    <p><b>Population:</b> ${e.population}</p>
    <p><b>Gravity:</b> ${e.gravity}</p>
    <p><b>Terrain:</b> ${e.terrain}</p>
  `,r.style.display="block"}async function q(e){const n=await(await fetch("https://swapi.info/api/planets")).json(),a=n.find(s=>s.name.toLowerCase()===e.toLowerCase())||n.find(s=>s.name.toLowerCase().includes(e.toLowerCase()));if(!a)throw new Error("Не знайдено");return a}Te.forEach(e=>{e.addEventListener("click",async()=>{const t=e.dataset.name;try{const n=await q(t);C(n)}catch{b.textContent="Помилка",I.textContent="Не вдалося завантажити дані",r.style.display="block"}})});S.addEventListener("click",async()=>{const e=M.value.trim();if(e)try{const t=await q(e);C(t)}catch{b.textContent="Помилка",I.textContent="Планету не знайдено",r.style.display="block"}});M.addEventListener("keydown",e=>{e.key==="Enter"&&S.click()});Re.onclick=()=>{r.style.display="none"};window.onclick=e=>{e.target===r&&(r.style.display="none")};
